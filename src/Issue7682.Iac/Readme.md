...
Notes:
* `app` is an abbreviation for Issue7682, use the acronym of your choice and replace app with that (for example abc) using exact match - case sensitive find and replace in this file and AdStack.cs.
...
pulumi config set Issue7682.Iac:sql-server-app-db-admin-id
pulumi config set Issue7682.Iac:sql-server-app-db-admin-password --secret

# Provide SMTP server's host, port, user, pass and default email sender.
pulumi config set Issue7682.Iac:default-email-from
pulumi config set Issue7682.Iac:email-server-host
pulumi config set Issue7682.Iac:email-server-port
pulumi config set Issue7682.Iac:email-server-userName
pulumi config set Issue7682.Iac:email-server-password --secret

# Create and provide an identity certificate password
pulumi config set Issue7682.Iac:identity-certificate-password --secret
```
